...
// the program assumes that there is an LEAccount
// for creating LEAccounts see dir LEAccount
//
...
func InitCertLib(dbg bool, domain string, prod bool, chal string)(certobj *CertObj, err error) {


	if len(domain) == 0 {return nil, fmt.Errorf("error -- no specified domain!\n")}
	...
	default:
		return nil, fmt.Errorf("invalid challenge: %s\n",chal)
	}

	certObj := CertObj{}
	certObj.Dbg = dbg
	certObj.Domain = domain
	certObj.Prod = prod
	...
	if err !=nil {return nil, fmt.Errorf("could not find dir %s: %v!", devopsDir, err)}

	certObj.CertDir = devopsDir + "/devops"
	certObj.CsrDir = devopsDir + "/devops"

	ndom := make([]byte,len(domain))
	ndom = []byte(domain)
	for i:=0;i< len(domain); i++ {
		if ndom[i]=='.' {ndom[i] = '_'}
	}

	certObj.CertName = string(ndom)
	if chal == "http" {return &certObj, nil}
...
//    fmt.Printf("crdat: %s\n", crdat)
    ...
    acntFilnam := certobj.LeDir + "/keys/azulLEProd.yaml"
    if !certobj.Prod {acntFilnam = certobj.LeDir + "/keys/azulLETest.yaml"}
...
func (certobj *CertObj) CreateCerts(cr *CrObj)(derCerts [][]byte, err error) {

	client := certobj.Client
	ctx := certobj.Ctx

    certKey, err := ecdsa.GenerateKey(elliptic.P256(), rand.Reader)
    if err != nil {return nil, fmt.Errorf("ecdsa.GenerateKey: %v\n",err)}
    ...
    if err != nil {return nil,fmt.Errorf("error -- certLib.SaveKeypem: %v",err)}
	...
	if err != nil {return nil, fmt.Errorf("CreateCertReq: %v",err)}
	if certobj.Dbg {
...
//		PrintCertReq(certReqB)
	...
	if err != nil {return nil, fmt.Errorf("invalid signature of cert req!")}
    ...
    if err != nil {return nil, fmt.Errorf("CreateOrderCert: %v\n",err)}
    ...
    if err != nil {return nil, fmt.Errorf("SaveCerts: %v\n",err)}
...
func GetCertsExp(certFile string, dbg bool)(certsinfo []CertInfo, err error){

    pemData, err := os.ReadFile(certFile)
    if err != nil {return nil, fmt.Errorf("os.ReadFile: %v\n", err)}
    if dbg {fmt.Println("********** start pem file ********")}

    restStart := pemData

	certsinfo = make([]CertInfo, 0, 10)
    certCount:=0
    for i:=0; i< 10; i++ {
        block, rest := pem.Decode(restStart)
        if block == nil {
...
//        }
		...
		derByt := block.Bytes
        cert, err := x509.ParseCertificate(derByt)
        if err != nil {return nil, fmt.Errorf("x509.ParseCertificates: %v", err)}

		if cert.IsCA {continue}
...
//        PrintCertInfo(*cert, i)
		...
		if len(cert.DNSNames) > 0 {
			for k:=0; k< len(cert.DNSNames); k++ {
				fmt.Printf("  %d: %s\n", k+1, cert.DNSNames[k])
				names[k] = cert.DNSNames[k]
			}
		}
		certinfo.Names=names
...
func (certobj *CertObj) GetAuthFromOrder (CrList []CrObj, order *acme.Order) (crList []CrObj, err error) {
    ...
    for i:=0; i< numAcmeDom; i++ {
        ...
        if certobj.Dbg {
			fmt.Printf("debug -- Authorisation status: %s\n", auth.Status)
			PrintAuth(auth)
		}
...
func (certobj *CertObj) GetAuthDnsFromOrder (CrList []CrObj, order *acme.Order) (crList []CrObj, err error) {
